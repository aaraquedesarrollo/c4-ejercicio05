...
app.post("*", (req, res, next) => res.status(403).json({ error: true, mensaje: "Te pensabas que podías jaquearme" }));
app.put("*", (req, res, next) => res.status(403).json({ error: true, mensaje: "Te pensabas que podías jaquearme" }));
app.delete("*", (req, res, next) => res.status(403).json({ error: true, mensaje: "Te pensabas que podías jaquearme" }));
...
app.get("/metro/linea/:nombreLinea", async (req, res, next) => {
  ...
  const lineaEncontrada = lineasMetro.find((linea) => linea.properties.NOM_LINIA.toLowerCase() === nombreLinea.toLowerCase());
  if (!lineaEncontrada) {
    res.json({ error: true, mensaje: "No existe la linea buscada"});
  }
  const {features: paradasLinea} = await obtenerParadasLinea(lineaEncontrada.properties.CODI_LINIA);
  res.json(
    {
      linea: lineaEncontrada.properties.NOM_LINIA,
      descripcion: lineaEncontrada.properties.DESC_LINIA,
      paradas : paradasLinea.map(({properties: {ID_ESTACIO, NOM_ESTACIO}}) => ({
        id: ID_ESTACIO,
        nombre: NOM_ESTACIO
      }))
    }
